angular.module('page', ['blimpKit', 'platformView', 'platformLocale', 'EntityService'])
	...
	.controller('PageController', ($scope, EntityService, Extensions, LocaleService, ButtonStates) => {
		...
		let translated = {
			yes: 'Yes',
			no: 'No',
			deleteConfirm: 'Are you sure you want to delete ProductType? This action cannot be undone.',
			deleteTitle: 'Delete ProductType?'
		};
		...
		LocaleService.onInit(() => {
			...
			translated.deleteTitle = LocaleService.t('codbex-products:codbex-products-model.defaults.deleteTitle', { name: '$t(codbex-products:codbex-products-model.t.PRODUCTTYPE)' });
			translated.deleteConfirm = LocaleService.t('codbex-products:codbex-products-model.messages.deleteConfirm', { name: '$t(codbex-products:codbex-products-model.t.PRODUCTTYPE)' });
		});
			...
			EntityService.count(filter).then((resp) => {
				...
				request.then((response) => {
					...
					Dialogs.showAlert({
						...
						message: LocaleService.t('codbex-products:codbex-products-model.messages.error.unableToLF', { name: '$t(codbex-products:codbex-products-model.t.PRODUCTTYPE)', message: message }),
				...
				Dialogs.showAlert({
					title: LocaleService.t('codbex-products:codbex-products-model.t.PRODUCTTYPE'),
					message: LocaleService.t('codbex-products:codbex-products-model.messages.error.unableToCount', { name: '$t(codbex-products:codbex-products-model.t.PRODUCTTYPE)', message: message }),
					type: AlertTypes.Error
				});
				console.error('EntityService:', error);
			});
			...
			Dialogs.showDialog({
				...
				if (buttonId === 'delete-btn-yes') {
					EntityService.delete(id).then((response) => {
						...
						Dialogs.showAlert({
							title: LocaleService.t('codbex-products:codbex-products-model.t.PRODUCTTYPE'),
							message: LocaleService.t('codbex-products:codbex-products-model.messages.error.unableToDelete', { name: '$t(codbex-products:codbex-products-model.t.PRODUCTTYPE)', message: message }),
							type: AlertTypes.Error
						});
						console.error('EntityService:', error);
					});
