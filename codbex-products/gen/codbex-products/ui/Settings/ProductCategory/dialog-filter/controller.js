angular.module('page', ['blimpKit', 'platformView', 'platformLocale']).controller('PageController', ($scope, ViewParameters, LocaleService) => {
	...
	$scope.filter = () => {
		...
		if (entity.Path) {
			filter.$filter.contains.Path = entity.Path;
		}
		Dialogs.postMessage({ topic: 'codbex-products.Settings.ProductCategory.entitySearch', data: {
			entity: entity,
			filter: filter
		}});
		$scope.cancel();
	...
	$scope.cancel = () => {
		Dialogs.closeWindow({ id: 'ProductCategory-filter' });
	};
