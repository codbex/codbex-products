angular.module('page', ['blimpKit', 'platformView', 'EntityService'])
	...
	.controller('PageController', ($scope, $http, EntityService, Extensions, ButtonStates) => {
		...
		Extensions.getWindows(['codbex-products-custom-action']).then((response) => {
			$scope.pageActions = response.data.filter(e => e.perspective === 'Catalogues' && e.view === 'Catalogue' && (e.type === 'page' || e.type === undefined));
		});
		...
		Dialogs.addMessageListener({ topic: 'codbex-products.Catalogues.Catalogue.entityCreated', handler: () => {
			refreshData();
			$scope.loadPage($scope.dataPage, $scope.filter);
		}});
		Dialogs.addMessageListener({ topic: 'codbex-products.Catalogues.Catalogue.entityUpdated', handler: () => {
			refreshData();
			$scope.loadPage($scope.dataPage, $scope.filter);
		}});
		...
		$scope.loadPage = (pageNumber, filter) => {
			...
			EntityService.count(filter).then((resp) => {
				...
				EntityService.search(filter).then((response) => {
					...
					Dialogs.showAlert({
						title: 'Catalogue',
						message: `Unable to list/filter Catalogue: '${message}'`,
				...
				Dialogs.showAlert({
					title: 'Catalogue',
					message: `Unable to count Catalogue: '${message}'`,
					type: AlertTypes.Error
				});
				console.error('EntityService:', error);
			});
		...
		$scope.selectEntity = (entity) => {
			...
			Dialogs.postMessage({ topic: 'codbex-products.Catalogues.Catalogue.entitySelected', data: {
				entity: entity,
		...
		$scope.createEntity = () => {
			...
			Dialogs.postMessage({ topic: 'codbex-products.Catalogues.Catalogue.createEntity', data: {
				entity: {},
				optionsProduct: $scope.optionsProduct,
				optionsStore: $scope.optionsStore,
				optionsBaseUnit: $scope.optionsBaseUnit,
		...
		$scope.updateEntity = () => {
			$scope.action = 'update';
			Dialogs.postMessage({ topic: 'codbex-products.Catalogues.Catalogue.updateEntity', data: {
		...
		$scope.deleteEntity = () => {
			...
			Dialogs.showDialog({
				...
				message: `Are you sure you want to delete Catalogue? This action cannot be undone.`,
				buttons: [{
					id: 'delete-btn-yes',
				...
				if (buttonId === 'delete-btn-yes') {
					EntityService.delete(id).then(() => {
						...
						Dialogs.showAlert({
							title: 'Catalogue',
							message: `Unable to delete Catalogue: '${message}'`,
							type: AlertTypes.Error
						});
						console.error('EntityService:', error);
					});
