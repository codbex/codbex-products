angular.module('page', ['blimpKit', 'platformView', 'platformLocale', 'EntityService'])
	...
	.controller('PageController', ($scope, $http, EntityService, Extensions, LocaleService, ButtonStates) => {
		...
		let translated = {
			yes: 'Yes',
			no: 'No',
			deleteConfirm: 'Are you sure you want to delete CampaignEntry? This action cannot be undone.',
			deleteTitle: 'Delete CampaignEntry?'
		};
		...
		Extensions.getWindows(['codbex-products-custom-action']).then((response) => {
			$scope.pageActions = response.data.filter(e => e.perspective === 'Products' && e.view === 'CampaignEntry' && (e.type === 'page' || e.type === undefined));
			$scope.entityActions = response.data.filter(e => e.perspective === 'Products' && e.view === 'CampaignEntry' && e.type === 'entity');
		});
		...
		Dialogs.addMessageListener({ topic: 'codbex-products.Products.CampaignEntry.clearDetails', handler: () => {
			$scope.$evalAsync(() => {
				$scope.entity = {};
				$scope.action = 'select';
			});
		}});
		Dialogs.addMessageListener({ topic: 'codbex-products.Products.CampaignEntry.entityCreated', handler: () => {
			$scope.loadPage($scope.dataPage, $scope.filter);
		}});
		Dialogs.addMessageListener({ topic: 'codbex-products.Products.CampaignEntry.entityUpdated', handler: () => {
			$scope.loadPage($scope.dataPage, $scope.filter);
		}});
		Dialogs.addMessageListener({ topic: 'codbex-products.Products.CampaignEntry.entitySearch', handler: (data) => {
			resetPagination();
		...
		$scope.openDetails = (entity) => {
			...
			Dialogs.showWindow({
				...
				params: {
					action: 'select',
					entity: entity,
					optionsCampaign: $scope.optionsCampaign,
				},
			});
		};
			...
			Dialogs.showDialog({
				...
				if (buttonId === 'delete-btn-yes') {
					EntityService.delete(id).then(() => {
						$scope.loadPage($scope.dataPage, $scope.filter);
						Dialogs.triggerEvent('codbex-products.Products.CampaignEntry.clearDetails');
					}, (error) => {
						const message = error.data ? error.data.message : '';
						Dialogs.showAlert({
							title: LocaleService.t('codbex-products:codbex-products-model.t.CAMPAIGNENTRY'),
		...
		$scope.optionsCampaignValue = function (optionKey) {
			for (let i = 0; i < $scope.optionsCampaign.length; i++) {
				if ($scope.optionsCampaign[i].value === optionKey) {
					return $scope.optionsCampaign[i].text;
				}
			}
			return null;
		};
